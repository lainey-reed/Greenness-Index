...
extractVIs_edit2 <- function(df,files,file.name,img.path,roi.path,vi.path=NULL,
                             roi.name,plot=FALSE, begin=NULL, spatial=FALSE, 
                             date.code, npixels=1, file.type='.jpg', bind=FALSE, 
                             ncores='all', log.file=NULL) { 
  ...
  if (spatial==FALSE) {
    ...
    for (roi in roi.pos) { 
      ...
      temp.VI <- data.frame(filename=file.name,ROI=k,date = temp.date, 
                            ...
                            bi.sd = temp.bi.sd, plant=temp.plant)      
    ...
    if (is.null(begin)) {
      #writing the csv takes longer but is more practical to access the data
      write.csv(VI.data,file=paste(vi.path,file.name,'.VI.data.Rdata.csv',sep=''))
    } else { #kept this in for bind to be used if wanted
      if (bind) {
        VI.data.new <- VI.data
        write.csv(file=paste(vi.path,file.name,'.VI.data.Rdata.csv',sep=''))
        check <- VI.data.new[[1]]$date[1]<tail(VI.data[[1]]$date,1)
        if (check) warning('New begin date is prior to the end of the already
                           existing records in VI.data! Check your dates')
    ...
    for (roi in roi.pos) {    
      ...
      #if (!is.null(begin)) if (end < beg.date) stop('Your final date is later
      #than last record in your timeseries')
      end <- trunc(end, 'day')
...
# All pictures need to be named "filename_mm_dd_yy_HHMM.jpg" for this to work
...
all_data=data.frame(Filename=character(),ROI=integer(),Date=as.Date(character()),
                    ...
                    Blue_Index_SD=double(),plant=character())
...
for (i in 1:length(list_pictures)){
  ...
  all_data=extractVIs_edit2(all_data,list_pictures[i],file_name,'./Pics/', "./ROIs/", 
                            "./VIs/", NULL, plot = TRUE, begin = NULL, 
                            spatial = FALSE, date.code = "mm_dd_yy_HHMM", 
                            npixels=1, file.type='.jpg', bind=FALSE, 
                            ncores='all', log.file=NULL)
